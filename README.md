...
| SVM (RBF kernel) | 0.96 |
| Decision Tree (max_depth=3) | 0.94 |
| Random Forest (n_estimators=120, max_depth=5) | 0.92 |
| KNN (k tuned 1–20) | varies by k |
| AdaBoost (GridSearchCV tuned) | tuned via cross-validation |
| Neural Network (Keras, 2 hidden layers) | 0.80 |
...
- encoders.pkl — a dict of fitted `LabelEncoder`s, one per categorical column.
